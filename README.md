...
`shutdown()` 会停止接收新的 `submit`、`submitFor` 和 `post` 请求，并等待正在运行的任务和已排队的任务完成。已提交的任务不会被取消；关闭完成后再次提交会抛出 `std::runtime_error`。析构函数也会执行相同的关闭流程，因此通常可以依赖对象离开作用域时自动关闭线程池。应避免在线程池关闭前提交会永久阻塞的任务。
